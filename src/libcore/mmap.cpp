...
MemoryMappedFile::MemoryMappedFile(const fs::path &filename) : m_filename(filename) {
	...
	if (fd == -1)
		Log(EError, "Could not open \"%s\"!", m_filename.string().c_str());
	m_data = mmap(NULL, m_size, PROT_READ, MAP_SHARED, fd, 0);
	if (m_data == NULL)
		Log(EError, "Could not map \"%s\" to memory!", m_filename.string().c_str());
	if (close(fd) != 0)
		Log(EError, "close(): unable to close file!");
#elif defined(WIN32)
	m_file = CreateFile(filename.string().c_str(), GENERIC_READ, 
		FILE_SHARE_READ, NULL, OPEN_EXISTING, 
		FILE_ATTRIBUTE_NORMAL, NULL);
	if (m_file == INVALID_HANDLE_VALUE)
		Log(EError, "Could not open \"%s\": %s", m_filename.string().c_str(),
			lastErrorText().c_str());
	...
	if (m_fileMapping == NULL)
		Log(EError, "CreateFileMapping: Could not map \"%s\" to memory: %s", 
			m_filename.string().c_str(), lastErrorText().c_str());
