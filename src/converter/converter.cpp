...
void GeometryConverter::convert(const fs::path &inputFile, 
	...
	if (!fs::exists(textureDirectory)) {
		SLog(EInfo, "Creating directory \"%s\" ..", textureDirectory.native().c_str());
		fs::create_directory(textureDirectory);
	}

	if (!fs::exists(meshesDirectory) && !m_packGeometry) {
		SLog(EInfo, "Creating directory \"%s\" ..", meshesDirectory.native().c_str());
	...
	if (!adjustmentFile.empty()) {
		...
		XERCES_CPP_NAMESPACE_QUALIFIER DOMDocument *adj = parser->parseURI(adjustmentFile.native().c_str());
		if (adj == NULL)
		...
		if (ofile.fail())
			SLog(EError, "Could not write to \"%s\"!", outputFile.native().c_str());
		ofile << os.str();
		ofile.close();
	}
	if (m_geometryFile) {
